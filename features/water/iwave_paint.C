...
#endif
#include <iostream>
#include <cmath>    // for exp, sqrt, j0
using namespace std;
int iwidth, iheight, array_size; // renamed from 'size' to avoid conflict with std::size
const int p = 6; // kernel radius
float *display_map;
float *obstruction;
float *source;
float *height;
...
void Initialize(float *data, int array_size, float value)
{
...
// p pixels all the way around act like a hard wall.
//
void ComputeVerticalDerivative()
{
	// first step: the interior
	for (int ix = p; ix < iwidth - p; ix++)
	{
		for (int iy = p; iy < iheight - p; iy++)
		{
			int index = ix + iwidth * iy;
			float vd = 0;
			for (int iix = -p; iix <= p; iix++)
			{
				for (int iiy = -p; iiy <= p; iiy++)
				{
					int iindex = ix + iix + iwidth * (iy + iiy);
					vd += kernel[iix + p][iiy + p] * height[iindex];
				}
			}
...
void cbOnKeyboard(unsigned char key, int x, int y)
{
	...
	case 'b':
		ClearWaves();
		ClearObstruction();
		Initialize(source, array_size, 0.0);
		break;
	default:
		break;
	}
...
int main(int argc, char **argv)
{
	...
	height = new float[array_size];
	previous_height = new float[array_size];
	vertical_derivative = new float[array_size];
	obstruction = new float[array_size];
	source = new float[array_size];
	display_map = new float[array_size];
	ClearWaves();
	ClearObstruction();
	ConvertToDisplay();
	Initialize(source, array_size, 0);
	InitializeBrushes();
